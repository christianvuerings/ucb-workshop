...
object JavaJaxRSServerGenerator extends BasicJavaGenerator {
  ...
  override def templateDir = "src/main/templates/java-jaxrs"

  val outputFolder = "server/java-jaxrs"
  ...
  override def typeMapping = super.typeMapping ++ Map("File" -> "InputStream")
                ...
                case Some(pattern(valueType, values)) => {
                  valueType match {
                    case "LIST" => {
                      ...
                      Some(l.mkString(quote, quote + "," + quote, quote))
                    }
                    case "RANGE" => {
                      val r = values.split(",")
